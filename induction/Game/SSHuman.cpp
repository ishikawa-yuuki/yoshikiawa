...
void SSHuman::Move()
{
	...
	const float diff_true = 100.0f * 100.0f;
	const float div = diff.LengthSq() / (400.0f * 400.0f) * 12.0f;
	if (diff.LengthSq() <= diff_true) {
...
void SSHuman::Turn()
{
	...
	if (fabsf(m_movespeed.x) <= range   //fabsfは絶対値。m_movespeed.x&m_movespeedzが
		&&fabsf(m_movespeed.z) <= range) {//0.001以下の時には何もしない。
		return;
	}
	else {
...
void SSHuman::AnimeControll()
{
	...
	if (m_movespeed.LengthSq() > run_true) {
		...
		m_skin->PlayAnimation(enAnimationClip_walk, time);
	}
	else {
